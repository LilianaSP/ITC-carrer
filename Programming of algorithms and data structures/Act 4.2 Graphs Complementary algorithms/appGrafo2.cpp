...
public:
    ...
    bool isTree(); //This function checks if the graph is a tree
    //We create the topological sort function
    void topologicalSort();
    void topologicalSortUtil(int, bool visited[], stack<int> &Stack);
...
//------------Function isTree (Check if the graph is a tree)--------------
//  isTree Function complexity - O(V + E) - Space Complexity O(V)
bool Graph::isTree(){
    ...
    this -> ResetVisited();
    if(!this -> isBipartite(0)){
        return false;
    }

    //We check if the graph is acyclic
    this -> ResetVisited();
    int count = 0;
    for(int i = 0; i < this -> numVertices; i++){
        if(!this -> visited[i]){
...
//------------Function topologicalSortUtil (topologicalSortUtil)--------------
//  topologicalSortUtil Function complexity - O(V + E) - Space Complexity O(V)

void Graph::topologicalSortUtil(int v, bool visited[], stack<int> &Stack){
    //We mark the current node as visited
    visited[v] = true;

    //We recursively call the function for all the vertices adjacent to this vertex
    list<int>::iterator i;
    ...
    //We push the current vertex to the stack
    Stack.push(v);
}

//------------Function topologicalSort (Topological Sort)--------------
//  topologicalSort Function complexity - O(V + E) - Space Complexity O(V)
void Graph::topologicalSort(){
    stack<int> Stack;

    //We mark all the vertices as not visited
    bool *visited = new bool[this -> numVertices];
    for(int i = 0; i < this -> numVertices; i++){
        visited[i] = false;
    }

    //We call the recursive function to store the topological sort
    for(int i = 0; i < this -> numVertices; i++){
        if(visited[i] == false){
            topologicalSortUtil(i, visited, Stack);
        }
    ...
    //We print the contents of the stack
